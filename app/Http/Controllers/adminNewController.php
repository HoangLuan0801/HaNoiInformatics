...
class adminNewController extends Controller
{
    ...
    public function postCreate(Request $req)
    {
        ...
        [
            ...
            "title.max"=>"Tiêu đề bản tin nhiều nhất 200 kí tự",
            "title.min"=>"Tiêu đề bản tin ít nhấn 10 kí tự",
        ...
        $new->alias = Uri::uri_friendly($req->title);
       ...
       // dd($req->files);
        if($req->hasFile('img_new'))
        {
            ...
            copy('client_assets/upload/images/new/'.$img, 'admin_assets/upload/images/new/'.$img);
        ...
        if($req->tags !="")
        {
            ...
            {
                $get_tag = Tag::where('tag_id',Uri::uri_friendly(trim($arr_tag[$i])))->get();
                if(count($get_tag) > 0)
                {
                    $new_tag = new New_tag;
                    $new_tag->tag_id = Uri::uri_friendly(trim($arr_tag[$i]));
                ...
                {
                    $tag = new Tag;
                    $tag->tag_id = Uri::uri_friendly(trim($arr_tag[$i]));
                    $tag->name = $arr_tag[$i];
